...
class Qnet5(nn.Module):
    def __init__(self, Num_packet, Num_file, F_packet, node, output_size):
        super(Qnet5, self).__init__()  # n x 5 x 160 -->5
        self.node = node
        ...
        self.Lc = nn.Linear(1, 1, bias=False).to(device)
    ...
    def forward(self, x):
        ...
        prop = x[:, 5, :4] * 1
        x_s = x[:, :5] * 1
        x_s[:, 0] += self.Lc(prop[:, 0].unsqueeze(1))
        x_s[:, 1] += self.Lc(prop[:, 1].unsqueeze(1))
        x_s[:, 2] += self.Lc(prop[:, 2].unsqueeze(1))
        x_s[:, 3] += self.Lc(prop[:, 3].unsqueeze(1))
