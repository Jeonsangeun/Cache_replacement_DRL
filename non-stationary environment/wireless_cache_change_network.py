...
class cache_replacement:
    ...
    def __init__(self, coverage, Zipf_ex, Mem, non_factor):
        ...
        self.hit = 0
        self.TTL_time = 1000
        self.change = non_factor
        self.change_contents = np.array(range(0, self.F_packet, self.Num_packet))

    def change_pop(self): # non-stationary func
        Top_tier = self.change_contents[:self.change]
        Tip_tier = np.delete(self.change_contents, range(self.change))
        self.change_contents = np.append(Tip_tier, Top_tier)
        self.Setting = tuple(self.change_contents)
    ...
    def reset(self): # init episode
        ...
        self.MS_error = self.Probabilistic_FH(d)
